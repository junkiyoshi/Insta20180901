...
void ofApp::update() {
	...
	for (int i = 0; i < this->circles.size(); i++) {
		...
		for (int j = i + 1; j < this->circles.size(); j++) {
			...
			if (distance < 80) {
				...
				this->circles[i]->addForce(this->circles[i]->getPosition() - this->circles[j]->getPosition(), 0.1);
				this->circles[j]->addForce(this->circles[j]->getPosition() - this->circles[i]->getPosition(), 0.1);
			}
		}
	}
	...
	for (int i = 0; i < this->circles.size(); i++) {
		...
		float distance = this->circles[i]->getPosition().distance(mouse);
		if (distance < 80) {

			this->circles[i]->addForce(this->circles[i]->getPosition() - mouse, 10.0);
		}
	}

	for (int i = 0; i < this->circles.size(); i++) {

		this->sizes[i] = ofMap(this->circles[i]->getVelocity().length(), 0, 10, 0, 1);
	}
